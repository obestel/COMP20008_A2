...
Y['TotalCount'] = np.log(Y['TotalCount'])
...
axs[0, 0].set_title('MEAN_INCOME VS Log(TotalCount)')

axs[0, 1].scatter(ptv_income['MEDIAN_INCOME'], Y)
axs[0, 1].set_title('MEDIAN_INCOME VS Log(TotalCount)')

axs[1, 0].scatter(ptv_income['POP_DENSITY_KM2'], Y)
axs[1, 0].set_title('POP_DENSITY_KM2 VS Log(TotalCount)')

axs[1, 1].scatter(ptv_income['LAND_AREA_KM2'], Y)
axs[1, 1].set_title('LAND_AREA_KM2 VS Log(TotalCount)')

plt.tight_layout()

plt.suptitle('Variables vs Log(TotalCount)')
plt.subplots_adjust(top=0.93)
plt.savefig('Variables_vs_TotalCount_log')
...
m1 = 'Log(TotalCount) = ' + str(model1.intercept_).lstrip('[').rstrip(']') +' + '+ coef[0] + '*MEAN_INCOME'
...
m2 = 'Log(TotalCount) = ' + str(model2.intercept_).lstrip('[').rstrip(']') +' + '+ coef[0] + '*MEDIAN_INCOME' \
...
m3 = 'Log(TotalCount) = ' + str(model3.intercept_).lstrip('[').rstrip(']') +' + '+ coef[0] + '*MEAN_INCOME' + '\n' \
...
m4 = 'Log(TotalCount) = ' + str(model4.intercept_).lstrip('[').rstrip(']') +' + '+ coef[0] + '*MEAN_INCOME' \
...
m5 = 'Log(TotalCount) = ' + str(model5.intercept_).lstrip('[').rstrip(']') +' + '+ coef[0] + '*MEDIAN_INCOME' \
+ ' + '+ coef[1] + '*POP_DENSITY_KM2' \
...
m6 = 'Log(TotalCount) = ' + str(model6.intercept_).lstrip('[').rstrip(']') + ' + '+ coef[0] + '*POP_DENSITY_KM2' \
...
plt.subplots_adjust(top=0.93)
plt.savefig('Residuals_graphs_log')
